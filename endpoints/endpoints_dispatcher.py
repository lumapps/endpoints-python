...
class EndpointsDispatcherMiddleware(object):
    ...
    """
        ...
        for base_path in self._backend.base_paths:
            ...
            self._add_dispatcher(
                "%sstatic/.*$" % base_path, self.handle_api_static_request
            )
        ...
        if api_config_response:
            ...
            raise api_exceptions.ApiConfigurationError(
                "get_api_configs() returned no configs"
            )
    ...
    def _get_explorer_base_url(self, protocol, server, port, base_path):
        ...
        url = (
            "{0}://{1}:{2}/{3}".format(protocol, server, port, base_path.lstrip("/\\"))
            if show_port
            else "{0}://{1}/{2}".format(protocol, server, base_path.lstrip("/\\"))
        )
    ...
    """
        logging.info("Request just received to Cloud Endpoints")
        request = api_request.ApiRequest(environ, base_paths=self._backend.base_paths)
        ...
        if request.http_method == "OPTIONS":
            ...
            if cors_handler.allow_cors_request:
                ...
                return util.send_wsgi_response(
                    "200", [], "", start_response, cors_handler
                )
    ...
    """
        if request.path == PROXY_PATH:
            return util.send_wsgi_response(
                "200 OK", [("Content-Type", "text/html")], PROXY_HTML, start_response
            )
        else:
    ...
    def prepare_backend_environ(
        self, host, method, relative_url, headers, body, source_ip, port
    ):
    ...
    """
        ...
        transformed_environ = self.prepare_backend_environ(
            ...
            "POST",
            url,
            transformed_request.headers.items(),
            transformed_request.body,
            transformed_request.source_ip,
            orig_request.port,
        )
    ...
    class __CheckCorsHeaders(object):
        ...
        def __check_cors_request(self, request):
            ...
            if self.origin and (
                (self.cors_request_method is None)
                or (self.cors_request_method.upper() in _CORS_ALLOWED_METHODS)
            ):
        ...
        def update_headers(self, headers_in):
            ...
            headers[_CORS_HEADER_EXPOSE_HEADERS] = ",".join(
                tuple(_CORS_EXPOSED_HEADERS)
            )
    ...
    """
        ...
        for header, value in response_headers:
            if header.lower() == "content-type" and not value.lower().startswith(
                "application/json"
            ):
                return self.fail_request(
                    orig_request, "Non-JSON reply: %s" % response_body, start_response
                )
        ...
        # generate a HTTP 204.
        empty_response = self.check_empty_response(
            orig_request, method_config, start_response
        )
    ...
    """
        ...
        response_status = "%d %s" % (
            status_code,
            httplib.responses.get(status_code, "Unknown Error"),
        )
        cors_handler = self._create_cors_handler(orig_request)
        return util.send_wsgi_response(
            response_status, headers, body, start_response, cors_handler=cors_handler
        )
